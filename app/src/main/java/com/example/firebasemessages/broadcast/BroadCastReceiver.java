...
public class BroadCastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (Telephony.Sms.Intents.SMS_RECEIVED_ACTION.equals(intent.getAction())) {
            ...
            if (parts.length == 5 && Pattern.matches("([0-9]+)[.]([0-9]+)", strLatitude)
                ...
                notificationManager = NotificationManagerCompat.from(MapsFragment.getInstance().requireContext());
                sendOnChannel1("FirebaseMessages", "Ajout d'un SMS à la map", context);
            }
        }
    }
    ...
    public void sendOnChannel1(String title, String message, Context context) {
        ...
        Notification notification = new NotificationCompat.Builder(
                MapsFragment.getInstance().requireContext(),
                CHANNEL_1_ID)
